...
| **Robustness on real geometry** | The σ_v dose-response replicates on the 5-lane US-101 replica (−8.2% at 1% → −53.2% at 20%, all resolved). The *no-cost* result does **not**: that saturated site shows a resolved throughput cost of 0.7% (1% penetration) to 2.7% (20%) measured on the replica, and +1.4…+2.7% fuel at 1–10% penetration. The throughput cost was first published as −0.3…−1.6%, measured upstream of the replica (correction of 2026-09-25, [US101_PENETRATION.md](docs/US101_PENETRATION.md)) |
...
| Controller comparison at 5% / 100% | Changes of the means relative to the baseline mean: FollowerStopper (σ_v −61.2%, waves −96.1%) and JAD under a realistic 30 s/±20% detection oracle (−60.6%, −90.9%) are statistically tied; faithful PI-saturation trails at −29.7%. All resolved, no throughput cost ([CONTROLLER_COMPARISON.md](docs/CONTROLLER_COMPARISON.md)) |
| Detection realism | JAD is *unreliable with a perfect oracle* — it chatters and worsens 5/20 seeds by wave count (1/20 by σ_v); 30–60 s latency with ±20% noise removes the failure entirely ([JAD_ORACLE_RESULTS.md](docs/JAD_ORACLE_RESULTS.md)) |
...
6. **Controller caveats.** JAD with a *perfect* oracle is bimodal: it chatters
   and worsens 5 of 20 seeds by wave count, so the headline JAD number is the 30 s / ±20%
