...
*Note 2026-09-25 (wave row detector):* the wave column above and the `wave_speed` criteria rows of `artifacts/us101_validation_calibrated.json` (6.06 and 5.86 km/h) are the standard 40 km/h detector's reading of the site-clipped field, although the committed rows name the profile's stack detector and add "caller did not state which detector produced the value" (docs/PAPER_DRAFT.md Appendix C item 18). `scripts/m3_us101_validate.py` now scores the row as `scripts/i24_validate.py` does: the `fhwa_default` profile's detector on its own bins over each replicate's field clipped to the 640 m site, the mean over the replicates with a backward reading (NaN, failing, "no backward wave detected" when none has one), with the detector named to `evaluate`; the standard and stripe readings stay in the results as labelled diagnostics (results schema 3, observed cache v4). The committed artifact predates the change and is to be regenerated on the next VM round (stage `battery_us101` of `scripts/gcp/pipeline_i24.sh`). The stack reading of the site-clipped field has not been computed yet, so no new number is given here; the report's stack reading (`docs/reports/us101_replica/report.md`) covers each run's whole simulated road, not the site.

*Note 2026-09-25 (the regenerated artifact, VM AD):* stage `battery_us101` re-ran both arms at 20 seeds on VM AD (`flowstate-us101v`, n2-standard-8, self-deleting; commit 52207f3, which includes the detector fix of 9fef785) and regenerated `artifacts/us101_validation_calibrated.json` and `docs/reports/us101_replica/report.md`. The runs reproduce the committed ones: GEH 55.6 % / 22.2 % and RMSPE 35.9 % / 28.4 % are unchanged to the digit, and the standard detector's diagnostic reads 6.06 / 5.86 km/h as before. The `wave_speed` row, now measured with the profile's stack detector on the 640 m site-clipped field and naming it, reads **NaN and FAILS on both arms: no backward wave detected**, with 0 of 20 replicates giving a backward reading ("contrast below floor" in all 20). The observed NGSIM field gives no stack reading either (0 backward fronts, contrast below the floor). On this 640 m site the criterion therefore cannot discriminate between the model and the data; its FAIL says the detector found no front, not that the model's fronts are too slow. The verdict of every row is unchanged. The report changes only in its timestamp, the per-run wall times (n2-standard-8 against n2-standard-32) and a new insertion line (51,833 of 51,840 planned vehicles departed over 20 runs).
