...
docs/WEAVE_MODEL_PLAN.md, dated section; `WEAVE_DEFAULTS["exit_priority_onset"]`,
default 0 = off, hash-neutral unless set; measured and left off): an exiter
still owing its change has the exit priority (`_weave_choose_gap`'s
`priority`) from the step on which its distance to the gore is within the
onset of its own model's braking for the end of its lane, latched, as well as
once its forced change is due. The onset (`microsim.runner._weave_brake_onset_m`)
is the stop term of the SUMO car-following model the fleet runs
(`FleetSpec.model`, carried as the section state's `cf_model`): EIDM `v·T +
v²/(2·√(a·b)) + 0.05` m (the IIDM brakes towards a stop once its desired gap,
which for a stop carries no `minGap`, reaches the distance;
`SUMO_EIDM_POS_ACC_EPS_M` = 0.05 m is the model's `EIDM_POS_ACC_EPS`; `inf`
above the desired speed), IDM `(v·T + v²/(2·√(a·b))) / √(1 − (v/v0)⁴)` (`inf`
from `v0` on), at the vehicle's own `tau`, `accel`, `decel` and speed, `v0`
its `min(maxSpeed, lane limit)`; both read from SUMO 1.27.1's
`MSCFModel_EIDM.cpp` / `MSCFModel_IDM.cpp` and checked on the installed
binary with `vehicle.getStopSpeed` (the model's two sub-steps put the onset
1.3–3.3 m farther out). The forced change keeps `force_within_m` /
`force_after_s`, the yields at the lane ends keep the zone's due, and with
`ramp_outlet` set the onset priority's gap choice passes over the outlet's
vehicles (the zone's priority stays exempt). `weave_sections[i].n_onset_priority`
(the **43rd key**; `WeaveSectionDiagnosticsOut`, null for an older meta, not
shown by the dashboard; aggregated by `WEAVE_FIELDS`) counts the exiter-steps
with the onset priority before the zone's. On the corridor section test's
fixture (seeds 3 / 4 / 5, with `ramp_outlet`) the exiters' crossings into the
auxiliary lane in [51, 305) m in minutes 1–4 are made at a median 17.0 /
16.7 / 12.2 m/s against 15.7 / 17.0 / 11.9, the exit end's lanes read at or
below 20 m/s in 11 / 10 / 13 of 16 windows against 12 / 12 / 13, and T.H.52
departs 398 / 391 / 359 of 407; on the 29-run grid exits fall 6,035 → 5,965,
the T.H.52 capacity fixture's no-lock pin fails at seeds 3 and 4, and the
...
Since 2026-09-26 (WP-94) the artifact also carries, additively (schema unchanged), each seed's
`per_seed[i].n_collisions` and a pooled `collisions` block; the report gains a **Model integrity**
section. Both are specified in "Model integrity: collisions in the battery artifact and the report"
...
## Lane-change relaxation: the partner speed (WP-91) — 2026-09-25

WP-91 (docs/WEAVE_MODEL_PLAN.md, dated section). An addition to `calibration.lane_change_relaxation`: each side also carries the speed difference between its two vehicles. It is additive: every existing value, key and default is unchanged (the corridor section fixture's artifact reproduces WP-88's key for key), and no config field, run output, hash or golden changes.

- *`PARTNER_SPEEDS`* = `("rel_speed_ms",)`. `rel_speed_ms` is `front_v_ms − rear_v_ms` [m/s]: on the leader side the new leader's speed minus the changer's, on the follower side the changer's speed minus the new follower's. It is positive while the side's gap opens. At offset 0 it is minus the record's `lead_closing_ms` (leader side) or `lag_closing_ms` (follower side).
- *`post_change_gaps`* fills `values[side]["rel_speed_ms"]`, an `(n_events, n_offsets)` array, NaN exactly where the side was not read (it is read below `min_speed_ms` too, unlike the time gap). `with_population_ratio` and `concat_results` carry it; `to_long()` has a `rel_speed_ms` column. `MEASURES`, `SPEEDS` and `RATIOS` are unchanged.
- *`summarize_relaxation`*: each row also has `front_v_ms_p50` (per offset) and `rel_speed_ms` (`n, p25, p50, p75` per offset). A `PostChangeGaps` made without the array (built by hand) is summarized from its two speeds.
- *Artifacts.* `scripts/lane_change_relaxation.py` is unchanged; its summaries (`summary_by_zone_kind`, `summary_by_zone_kind_arrival_crossing`) carry the two keys from the next run on. Its `schema_version` stays 1 and its `measures` list stays `MEASURES`.

## Coverage thinning (`calibration.thinning`, `scripts/coverage_thinning.py`) — 2026-09-25

WP-91 (docs/WEAVE_MODEL_PLAN.md, dated section). A complete trajectory table thinned to look partially tracked, so the effect of I-24 MOTION's coverage (about 0.5–0.65 of the peak vehicle-time, as fragments; docs/I24_DATA.md §2, §4) on a measure can be measured on NGSIM US-101, which is complete in coverage. Pure functions of the frame, the fraction and the seed; nothing else in the package changes.

**`thin_vehicles(df, fraction, *, seed) -> ThinnedFrame`.** Keeps `round(fraction · n)` of the `n` vehicle ids (at least one), drawn without replacement by `flowstate_core.rng.make_rng(seed)` over the sorted ids, with their whole tracks and their own ids.

**`thin_fragments(df, fraction, *, seed, fragment_median_s=9.9, fragment_sigma=I24_FRAGMENT_SIGMA, gap_sigma=None, burn_in_s=None, id_prefix="frag-") -> ThinnedFrame`.**
- *The process.* Per vehicle (sorted-id order), an alternating renewal process of tracked spells (log-normal, median `fragment_median_s`, scale `fragment_sigma`) and untracked spells (log-normal, scale `gap_sigma` or `fragment_sigma`, mean `E[on] · (1 − fraction) / fraction`; zero length at `fraction = 1`), started `burn_in_s` before the vehicle's first sample (default the larger of 300 s and 20 mean cycles), so the expected tracked share is `fraction`.
- *The rows.* A row is kept when its `t` lies in a tracked spell `[start, end)`. Each (vehicle, spell) holding a kept row is a fragment and gets a new id, `id_prefix` plus a zero-padded number from a seeded permutation.
- *The I-24 parameters.* `I24_FRAGMENT_MEDIAN_S` = 9.9 s and `I24_FRAGMENT_HISTOGRAM` (the eight committed duration bins, 576,511 fragments) are docs/I24_DATA.md §2's. `I24_FRAGMENT_SHARE_GE_30S` = 62,784 / 576,511. `I24_FRAGMENT_SIGMA` = `lognormal_sigma(9.9, 30.0, I24_FRAGMENT_SHARE_GE_30S)` = 0.8996. The model's p90 is 31.36 s against the committed 31.4 s (`I24_FRAGMENT_P90_S`), and `lognormal_bin_shares()` gives each bin's observed and model shares (within 0.018 of each other).

**`thin_frame(df, model, fraction, *, seed, options=None)`** dispatches on `model` in `THINNING_MODELS` = `("vehicle", "fragment")`; `options` are `thin_fragments`' keywords (the vehicle model takes none).

**`ThinnedFrame`.**
- `frame`: the kept rows of `df` in input order with a fresh `RangeIndex`; every column as in `df` except `veh_id` (replaced by the fragment id on the fragment model). No row is added or altered otherwise.
- `source_rows`: each kept row's position in `df` (increasing).
- `fragments`: one row per fragment (fragment model) or kept vehicle (vehicle model): `fragment_id`, `source_id` (as a string), `t_start`, `t_end`, `duration_s` (`t_end − t_start`), `n_rows`, `at_track_start`, `at_track_end` (it holds the vehicle's first / last sample: cut by the track, not by the process).
- `kept_time_fraction`: kept rows over rows (vehicle-time on a shared grid; NaN on an empty frame).
- `counts`: `n_rows`, `n_rows_kept`, `n_vehicles`, `n_vehicles_kept`, `n_fragments`. `parameters`: the model and its parameters (the fragment model's `fragment_mean_s`, `gap_mean_s` and `burn_in_s` included).
- *Errors.* `ValueError` on a missing `t` or `veh_id`, a fraction outside (0, 1], a non-positive scale or median, or a negative burn-in.

**What the models do not reproduce.** I-24's losses are correlated in space (camera boundaries, overpasses) and by lane (lane 1 at 0.70–0.76, interior lane 3 at 0.40–0.54), and it has duplicate fragments and position noise. Both models lose tracking independently of position, lane and neighbours. The untracked spells' distribution is an assumption: I-24 publishes no statistic of it.

**Artifact `artifacts/coverage_thinning_us101.json`** (written by `scripts/coverage_thinning.py`, the opt-in pipeline stage `us101_coverage_thinning` (17), which has not run yet; JSON, `allow_nan=False`, `schema_version` 1, `kind` `"observed_thinned"`; one key per line, every list of scalars on one line).
- *The run.* NGSIM US-101 as stage 16 reads it (`scripts/us101_data.load_us101`, per period, ids `<period>-<veh_id>`). The reference is the table as held. Each thinning model at each kept fraction (`--fractions`, default 1.0 0.65 0.5) is run on the same thinning seeds (`spawn_seeds(--seed, --n-seeds)`, default 20260926 and 5; each period's seed `spawn_seeds(thinning seed, n_periods)[i]`). The vehicle model at 1.0 is the identity and is not run. The weaving zone is the reference's in every condition.
- *Per condition and seed*: (a) `post_change_gaps` exactly as stage 16 (`ratio_pop` against the thinned table's own `normal_time_gaps`, `ratio_eq` at the `idm_us101` means), summarized per zone kind × movement with `n_boot=0` and no fits; (b) `lane_change_gaps` records with `AcceptanceParams` from the `acceptance` block of `artifacts/i24_lane_change_gaps.json` (VM X's), `summarize_gaps`; (c) `gap_sequences` for the entering and exiting changes in the weaving zone, `driver_gaps`, `rejected_points`, `select_drivers`, `fit_groups` (the reference with `--n-boot` refits, 200 by default; the thinned conditions with none).
- *Measure keys* (flat): `relax.<zone_kind>.<movement>.<speed_class>.<side>.<n_changes | n_measured | n_read@o | <m>.p50@o | ratio_pop.n@o | rel_speed_ms.p25@0 | rel_speed_ms.p75@0>` for `m` in `ratio_pop, ratio_eq, time_gap_s, space_gap_m, rel_speed_ms` and `o` in 0, 5, 10 (groups weave entering / exiting / through and basic through, speed class `all`, and `10<=v<20` for weave entering); `gaps.<zone_kind>.<movement>.<n | share_no_<side> | <side>_gap_m.p10 | .p50 | <side>_time_gap_s.p10 | .p50 | <side>_closing_ms.p50 | refused_share | refused_by.<term>>`; `cg.<movement>.<speed_class>.<n_drivers | share_with_rejected | joint.n_used | joint.n_inconsistent | joint.<side>.median_s | separate.<side>.n_used | separate.<side>.median_s>` (a median is null when unfitted, at a bound or degenerate); `tracking.<kept_time_fraction | kept_time_fraction.<period> | n_vehicles_kept | n_fragments | fragment_duration_s.p50 | fragment_duration_s.p50_untruncated | n_changes | n_walked | n_sequenced>`.
- *Keys.* `schema_version`, `kind`, `created_at`, `source`, `code`, `code_dirty`, `data_hash`, `data`, `data_version`, `rows` (per period), `method`, `parameters` (`models`, `fractions`, `n_seeds`, `master_seed`, `thinning_seeds`, `period_seeds`, `periods`, `n_boot_reference`, `critical_gap_seed`), `zones`, `equilibrium`, `acceptance`, `reading_rule` (a pointer to the dated section), `reference_check_stage16` (the reference's `ratio_pop` and `ratio_eq` medians at the change against `artifacts/lane_change_relaxation_us101.json`, and `all_equal`; null without that file), `reference_intervals` (the reference's bootstrap intervals of the fitted medians, keyed as `cg.*`), `results`, `limitations`, `wall_s_by_condition`, `wall_s`, `peak_rss_mb`.
- *`results`*: per measure key, `reference` and, per model and fraction (`"1"`, `"0.65"`, `"0.5"`), `per_seed`, `n` (finite values), `mean`, `min`, `max`, `ci95` (Student t over the seeds, conditional on this one dataset; null below two values) and `shift` (`mean − reference`).

**Pipeline.** Stage `us101_coverage_thinning` (17, opt-in) runs `scripts/coverage_thinning.py`; it reads `data/ngsim`, which the launch ships with `--data-set i24` when present. `scripts/gcp/ingest_pipeline_results.sh` installs `coverage_thinning_*` artifacts.

## Model integrity: collisions in the battery artifact and the report (WP-94) — 2026-09-26

WP-94 (roadmap C3, "harden the auto-report"). The I-94 WB reference battery recorded 15 collisions over 20 seeds in its runs' `meta.json` for days before anyone read them (docs/ONBOARDING_MNDOT.md §11, records VM AF and VM AG; WP-93): neither the corridor battery artifact nor the auto-report printed `n_collisions`. Both now do. It is additive: no config field, run output, hash or golden changes; every key and value the artifact had is byte-identical (pinned by `tests/test_validation/test_validation_corridor_battery_collisions.py`, and checked once against the pre-WP-94 script, `validation.battery` and `validation.report` on a stored three-seed tree), and `schema` stays `flowstate.corridor_validation/1`, the precedent of `insertion`, `weave_exits` and the link-hour tables.

- *What is read.* `meta.json` only: `n_collisions` and `collisions` (the entry above: a colliding pair counted once, when first detected; each run logs its first 50 events, `microsim.runner.COLLISION_LOG_MAX`), `n_vehicles_departed`, and the forced-change counters of `scripted_merges[i]` (`n_forced`, `n_changed`) and `weave_sections[i]` (`n_forced`, `n_changed_in`, `n_changed_out`). Runs record no teleport counter (the runner starts SUMO with `--time-to-teleport -1`) and no emergency-braking counter, so neither is reported.
- *`validation.battery.collision_count(meta)`*: `n_collisions`, or None when the key is absent (runs written before 2026-09-16, hand-written fixtures): not recorded, never 0. *`lane_edge(lane)`*: the SUMO lane id without its `_<index>` suffix (`-178547099#2_1` → `-178547099#2`, `:J3_0_0` → `:J3_0`); an id without a numeric suffix is returned unchanged.
- *`validation.battery.collision_summary(metas, *, labels=None)`*: None when no meta records `n_collisions`; otherwise `{n_runs, n_runs_recorded, runs_not_recorded, total, n_runs_with_collisions, runs_with_collisions: [{run, n}], per_run: {mean, lo95, hi95, n, underpowered}, rate: {per_vehicles, value, n_collisions, n_departed, n_runs}, n_logged, locations: [{lane, edge, n, pos_m_min, pos_m_max, runs}], definition}`. `total` = Σ `n_collisions` over the runs that record it. `per_run` = `validation.metrics.ci` over those per-run counts (two-sided 95 % t-interval, not clamped, so a rare count can give a lower bound below zero; `underpowered` below 20 runs). `rate.value` = `per_vehicles · n_collisions / n_departed`, `per_vehicles` = `COLLISION_RATE_PER_VEHICLES` = 1000, over the runs that record both `n_collisions` and `n_vehicles_departed` (NaN, written null, when none departed): collisions per 1,000 vehicles that entered the network. Departed vehicles rather than vehicle-km because both counters cover the whole run, warm-up included, whereas every distance in `validation.metrics.Metrics` is windowed. `locations` groups the logged events by lane (an event without one under `UNKNOWN_LANE` = `"unknown"`), most collisions first, then by lane id; `pos_m_min` / `pos_m_max` are positions along the lane [m] (non-finite ones skipped, null when none); `runs` lists labels in first-seen order. `n_logged` ≤ `total`; the difference is counted but not located. Labels default to each meta's `seed`; `labels` of another length is a `ValueError`. `definition` is `COLLISION_DEFINITION`.
- *`validation.battery.forced_change_summary(metas)`*: one row per scripted merge or weaving section, keyed by its on-ramp (`"<model> <position>"` when unnamed), in first-seen order: `{model: "scripted merge" | "weave section", ramp, n_runs, n_forced, n_changed}`, summed over the `n_runs` runs that record both counters (`n_changed` of a scripted merge; `n_changed_in + n_changed_out` of a weaving section). A forced change is one made under SUMO's forced lane-change mode, which refuses a change only on an overlap (`merge_params["force_guard"]` above). Empty when no run lists either model.
- *Battery artifact* (`scripts/corridor_battery.py`). `per_seed[i].n_collisions` (after `link_hours`; null when that seed's meta has no counter) and `collisions` (after `weave_exits`): `collision_summary` over the seeds' metas, labelled by seed, so `runs_not_recorded`, `runs_with_collisions[*].run` and `locations[*].runs` hold seeds; null when no seed records the counter. `build_artifact(..., metas=None)` takes the metas in seed order; without them both keys are null. `main` reads each seed's `meta.json` once (also under `--criteria-only`) for `weave_exits` and `collisions`, and prints a `collisions` console line beside the insertion line: `N over k replicate(s), m with any, r per 1,000 departed[; not recorded for j replicate(s)][; most on lane L (n logged)]`, or `not recorded (…)`. `notes` is unchanged. The forced-change rows are in the report only.
- *Report* (`validation.report.generate_report`; the PDF renders the same markdown, and `api.jobs.report_job` reports carry it too). A `## Model integrity` section between Provenance and Acceptance criteria, built by `_integrity_context` from the micro runs' metas, each run labelled by its directory relative to the run set: a fixed intro; a `Collisions:` bullet — the total over the runs that record the counter, the runs that do not (by name), the per-run mean and interval with its n and underpowered flag, and the rate with its numerator, denominator and run count (or `no rate: …`), or `not recorded — no run in this set carries the collision counter (…)` when none does; `Runs with collisions: <run> (<n>), …`; one `Forced lane changes at <ramp> (<model>): F of C completed change(s) over k run(s) were made under SUMO's forced lane-change mode, …` bullet per `forced_change_summary` row; and, when events are logged, a table `Lane | Edge | Collisions logged | Position along the lane [m] | Runs` (every lane), with a sentence stating how many collisions are counted but not located when `n_logged < total`. The Limitations list gains, first: with collisions, `This run set contains N SUMO collision(s) in k of n run(s) (<run> (<n>), …), at lane <lane> (edge <edge>, lo–hi m): <n>; …` naming up to `LIMITATION_MAX_LOCATIONS` = 5 lanes, then `m more lane(s) in the Model integrity table` and `u not located` when they apply; with no counter in any run, `No run in this set records a collision count, so a collision-free simulation is not established.`; with the counter missing from some runs, `Collisions were not recorded for j of n run(s) (<runs>); a collision-free simulation is not established for them.` Every number is passed in from the two summaries; the template body stays numeral-free.

## AV command handback (`AVSpec.emergency_handback`, WP-95) — 2026-09-26
