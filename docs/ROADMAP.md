...
Open / pending: VM H's 20-seed battery with the exit-side runner departed 0.743 (RMSPE 0.782; one seed locks; no GEH pass — docs/ONBOARDING_MNDOT.md §10), and the per-seed read (docs/ONBOARDING_MNDOT.md §11a) puts the head of every queue in the corridor's last 850 m — the 40648744 entrance, a plain lane-change merge carrying 890–1,150 veh/h inferred by conservation, feeding the T.H.52 weave, from the warm-up on; the locking seed locked there inside the warm-up — so the next measurement is a standstill map of that stretch in the first hour, as is and with that entrance on the scripted merge (VM I); the two earlier batteries on the regenerated corridor are recorded in docs/ONBOARDING_MNDOT.md §10 (VM F on the reset lane-change settings: departed 0.233 over 20 seeds; VM G on the corrected settings with the sixth derivation: 0.247, the lock at the T.H.52 exit end); the seventh weave derivation — the abreast entrant–lane-1 pair resolved
