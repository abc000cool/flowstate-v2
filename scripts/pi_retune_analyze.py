"""Analyse the PI-controller comparison (scripts/pi_retune_experiment.py).
...
``artifacts/pi_retune_summary.json``.
"""
...
def main() -> None:
    ...
    (REPO / "artifacts" / "pi_retune_summary.json").write_text(json.dumps(summary, indent=2))

    print(f"{'cell':<16}{'throughput':>22}{'sigma_v_temporal':>22}{'waves':>16}")
    for cell in ("baseline", "pi_meanfrac", "pi_saturation"):
