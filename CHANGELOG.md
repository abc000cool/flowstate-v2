...
- Zipper family against the canonical family (20 seeds each, criteria rows re-scored with the published sweep grid): `zip_ramps` RMSPE 34.2% (canonical `ramps` 34.8%), GEH < 5 on 23% of recommended-coverage link-hours (20%), stack wave speed 15.7 km/h (15.7); `zip_corrected` 37.7% / 22% / 16.1 (canonical 33.7% / 17% / 15.9). The zipper merge, with its junction gap fitted, moves neither failing row out of failure and leaves the wave row passing: the merge-admittance defect of docs/I24_VALIDATION.md §0.5 (k) stands, and the family is kept as a documented negative result, not as a replacement.
