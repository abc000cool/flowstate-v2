...
- Merge models on guessed acceleration lanes: when netconvert adds the lane along a short attach edge and runs it into the next edge's lane 0, the merge branch now ends it with a connection patch applied to the compiled net (`networks.accel_lane_end`, `lane_end_patch_file`, `patch_net`; `meta.json["ramps"][*].acceleration_lane_terminated`) so `scripted`, `zipper` and `acceleration_lane` apply; a lane that feeds a weave or runs on as a through lane is refused with the lane counts. On the MnDOT peak slice this took the scripted merge onto four of seven entrances; the three that lock (a weave into an exit, a 1,071 m added through lane at T.H.61 NB, a two-lane exit piece) are outside every current merge model, and the slice still gridlocks — recorded in `docs/ONBOARDING_MNDOT.md` §6.
...
- Stations S1063 (west of the I-494 east junction) to S97 (I-35E east junction): 9.9 km of inventory, 14 mainline stations at ~0.7 km spacing, 17 ramps, 3–5 lanes; nine weekdays (2026-09-01 … 09-17), 05:30–09:30 local, 5-min windows; every mainline station 100 % valid over the span (`data/mndot/mndot_i94_wb_stpaul/observations.json`).
- `scenarios/mndot_i94_wb_stpaul.yaml`: 11.8 km chain of 32 OSM edges, 31 of 31 stations placed within 22 m; demand from S1063 (peak 4,275 veh/h), 17 ramps (10 from live detectors or their shares, 5 by conservation, 2 outside the observed span set to zero), boundary from S97; driver population `artifacts/idm_i24_capacity.json` (the I-24 fit, stated as such — no Minnesota trajectory data exists to refit). Three ramp detectors read zero and one "exit" pair reads more than the mainline (a collector–distributor split), which is why ramp flows close the balance rather than trusting detectors alone (`artifacts/demand_mndot_i94_wb_stpaul.json`).
- Fundamental diagram `artifacts/fd_mndot_i94_wb_stpaul.json` from 300k per-lane one-minute samples: v_f 26.9 m/s, w −2.8 m/s, ρ_jam 0.210 veh/m, ρ_c 0.0199 veh/m, q_max 0.535 veh/s (bootstrap CIs in the artifact). A fit from 5-min station means fails the plausibility check (flat congested branch) — the loader keeps the raw cache for this reason.
